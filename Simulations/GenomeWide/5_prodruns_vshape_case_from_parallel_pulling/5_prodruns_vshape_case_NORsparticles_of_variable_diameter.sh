lmpfile=$(ls -1 5*.lmp | grep -v NEUT)

#Parameters of the NAD interactions depend on the NADs particles radius
sigma1=0.5
#sigma5=1.67 ; # NORs particles of diameter 100nm  
#sigma5=2.2 ; # 0.64 of the nuclear volume expected for Random CLose Packing (RCP)
sigma5=3.34 ; # NORs particles of diameter 200nm  
rad=$(echo $sigma5 | awk '{print int($1*30.*2)}')
echo $rad
...
#optimalCHrepulsion=0.0002 #0.0001953125 0.000390625 0.00078125 0.0015625 0.003125 0.00625 0.0125 0.025 0.05 0.50 1.0
...
#optimalCHattraction=0.20 #0.05 0.10 0.20 0.30 0.40 0.50 1.0
...
#optimalCHattraction=0.10   #0.025 0.0375 0.05 0.10 0.20 0.25 0.40
...
do
    ...
    do
        ...
        do
            ...
            do
                for scenarioFH in FH_ATTRandNEUT ; # FH_ATTRandNEUT FH_NEUTandNEUT ;
                do
		    for scenarioAC in AC_ATTRandNEUT ; # AC_NEUTandNEUT AC_ATTRandNEUT
		    do    
			for scenarioCH in CH_NEUTandREPU ; # CH_NEUTandNEUT CH_NEUTandREPU CH_ATTRandREPU CH_ATTRandNEUT ;
			do
			    ...
			    wdir=system_${sNAD}_${sAC}_${sFH}_${sCH}_NORsparticles_of_${rad}nm
			    echo $wdir
			    #continue
				...
				xcom=$(echo $com | awk '{print $1}') #V shape
				ycom=$(echo $com | awk '{print $2}') #V shape
				zcom=$(echo $com | awk '{print $3}') #V shape
...
# Reference case: all neutral
lmpfile=$(ls -1 5*.lmp | grep NEUT)

scenarioAC=AC_NEUTandNEUT
scenarioFH=FH_NEUTandNEUT
scenarioCH=CH_NEUTandNEUT

wdir=system_${sNAD}_${scenarioAC}_${scenarioFH}_${scenarioCH}
exit
mkdir -p ${wdir}
cd ${wdir}
echo ${wdir}
...
do
    ...
    sed -e "s/XXXdumpXXX/${dump}/g" -e "s/XXXtimestepXXX/${timestep}/g" -e "s/XXXrunXXX/${run}/g" -e "s/XXXreplicaXXX/${replica}/g" -e "s/XXXseedXXX/${seed}/g" -e "s/XXXsigma55XXX/${sigma55}/g" -e "s/XXXsigma15XXX/${sigma15}/g" -e "s/XXXrc55XXX/${rc55}/g" -e "s/XXXrc15XXX/${rc15}/g" -e "s/XXXR055XXX/${R055}/g" -e "s/XXXR015XXX/${R015}/g" -e "s/XXXxNADsXXX/${xcom}/g" -e "s/XXXyNADsXXX/${ycom}/g" -e "s/XXXzNADsXXX/${zcom}/g" -e "s/XXXNADattractionXXX/${NADattraction}/g" -e "s/XXXattractionXXX/${attraction}/g" -e "s/#${scenarioNAD}//g" -e "s/#${scenarioAC}//g" -e "s/#${scenarioFH}//g" -e "s/#${scenarioCH}//g" -e "s/XXXseedXXX/${seed}/g" -e "s/XXXreplicaXXX/${replica}/g" -e "s/XXXradiusXXX/${sigma5}/g" ../../${lmpfile} | grep -v scenario > _tmp.lmp
...
exit
# Parameters from Polymer coil-globule phase transition is a universal folding principle of Drosophila epigenetic domains Antony Lesage, Vincent Dahirel, Jean-Marc Victor, and Maria Barbi
