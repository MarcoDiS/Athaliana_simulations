...
int main(int argc,char** argv)
{
  ...
  int resolution, n_particles, n_bins;           /* Self explanatory variables */
  ...
  char line[200], filename_input[100];
  FILE *fp_input_files, *fp_input, *fp_output, *fp_error;


  fp_output = open_w("output.dat");
  ...
  while ((c = getopt (argc, argv, ":h:r:p:d:")) != -1)
    switch (c)
      {
      case 'r':
  ...
  n_bins = (int) n_particles/resolution;
  ...
  fp_input_files = open_r("DATA_FILES_INPUT.DAT");

  /* Computing the distance between fragments as in Giorgetti et al. */
  ...
  while( (fscanf(fp_input_files, "%s", &filename_input)) != EOF )
    { 
      ...
      fprintf(fp_error, "Computing the contacts between pairs of bins: ");
      for(i = 0; i < n_bins; ++i)
	{
	  for(j = i; j < n_bins; ++j)
	    {	      
	      ...
	      /*	      if(contact == 1) fprintf(stderr,"%d %d contact = %d\n",i,j,contact); */
...
void print_help()
{
  ...
  fprintf(stderr,"\t File DATA_FILES_INPUT.DAT contains the absolute paths of the coordinate files\n");
  fprintf(stderr," \n");
  fprintf(stderr,"OUTPUT:   \n");
  fprintf(stderr,"\t File output.dat contains the computed contact map\n");
